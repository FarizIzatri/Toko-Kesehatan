<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-th-foreground">Manajemen Pesanan (Shipping Order)</h1>
            <a href="{{ route('admin.orders.downloadSummary') }}" class="inline-flex items-center gap-2 bg-blue-500/10 text-blue-300 font-medium rounded-lg px-4 py-2 text-sm hover:bg-blue-500/20 transition-colors">
        ...
        @if ($orders->isEmpty())
            <div class="border border-th-border rounded-lg p-6 text-center">
                <p class="text-th-muted">Belum ada pesanan.</p>
            </div>
        @else
                ...
                <table class="min-w-full divide-y divide-th-border border border-th-border rounded-lg">
                    <thead class="bg-th-border/20">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-th-muted uppercase tracking-wider">Alamat</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-th-muted uppercase tracking-wider">Status</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-th-muted uppercase tracking-wider">Keterangan</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($orders as $order)
                            @php
                                ...
                                $statusText = [
                                    ...
                                    'paid' => 'Menunggu pengiriman vendor',
                                    'shipped' => 'Terkirim',
                                    'completed' => 'Selesai',
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-th-foreground">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                                ...
                                <td class="px-4 py-4 text-sm text-th-muted">
                                    {{ $statusText[$order->status] ?? ucfirst($order->status) }}
                                    <br>
                                    <small class="text-xs">Admin hanya dapat melihat status pesanan</small>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
